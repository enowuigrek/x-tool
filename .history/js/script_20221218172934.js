...
const displaySkuToCopyFromLinks = document.querySelector('.sku__to__copy__from__links'
);
...
const generateSkuFromLinks = () => {
  ...
  for (let pieceOfInputText of splitInputTextArray) {
    console.log(typeof pieceOfInputText);

    if (pieceOfInputText.startsWith("https://www.x-kom.pl/p/")) {
      console.log(skuFromLink.exec(pieceOfInputText));

      const skuFromInputText =
          skuFromLink.exec(pieceOfInputText)[1];
