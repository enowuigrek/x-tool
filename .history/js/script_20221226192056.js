...
const generateSkuFromLinks = () => {
  ...
  const noUndefinedSkuList = [];

  console.log(listSku);

  for (let sku of listSku) {
    if (sku == undefined) {
      indexUndefined = listSku.indexOf(sku);
      console.log(indexUndefinedlistSku);
    }
    listSku.splice(indexUndefinedlistSku, 1);
    noUndefinedSkuList.push(sku);
  }

  const noDuplicateSkuList = [...new Set(noUndefinedSkuList)];
