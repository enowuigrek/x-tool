...
const delateDuplicateAndUndefined = (listSku) => {
...
const generateSkuFromMessage = () => {
  ...
  for (let pieceOfInputText of splitInputTextArray) {
    if (pieceOfInputText.includes('https://www.x-kom.pl/p/' || 'https://al.to/p/'))
    ...
    if (
      ...
      pieceOfInputText === 'sku' ||
      pieceOfInputText === 'al.to:' ||
      pieceOfInputText === 'alto:'
    )
...
const displaySkuListLink = (listSku) => {
  ...
  resultLink = `<a href=https://www.x-kom.pl/szukaj?q=${skuToLink} target="blank"> Lista na stronie </a>`;
