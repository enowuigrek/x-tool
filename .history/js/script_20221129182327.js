...
function generateSku() {
  ...
  for (let singleLink of pastedSplitLinks) {
    ...
    let skuWitchZero = '00' + singleLink;
    console.log(skuWitchZero);

    if (parseInt(singleLink) && singleLink.length <= 7 && singleLink.length >= 4 || skuWitchZero) {
  ...
  for (let singleSku of listSku) {
    ...
    function addZero() {
      if (parseInt(singleSku) && singleSku.length > 6 && singleSku.length) {
