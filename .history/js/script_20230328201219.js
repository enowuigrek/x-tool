...
const generateSkuFromMessageArr = (input) => {
  ...
  const extractorSkuFromList = (list) => {
    const skuFromLink = new RegExp(/\/p\/(\d*)/);
    return skuFromLink.exec(list.href)[1];
  };
  
  const getProductListSKU = () => {
    const productListLinks = document.querySelectorAll('a.product-link');
    const listSkuArr = [];
  
    productListLinks.forEach(link => {
      const skuFromInputLink = extractorSkuFromList(link);
      listSkuArr.push(skuFromInputLink);
    });
