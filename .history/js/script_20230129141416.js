...
const orderLink = () => {
  ...
  if (orderNumber.length === 12) {
    ...
    if (linkBegin == undefined) {
      ...
      let resultOrderLink = `<a href= ${orderLink} target="_blank"> ${orderNumber} </a>`;
      if (orderNumber.startsWith(9)) {
        resultOrderLink = resultOrderLink.slice(0, -12);
      } else {
        displayOrderLink.innerHTML = resultOrderLink;
      }
    }
  } else if (orderNumber == ''){
    displayOrderLink.innerHTML = '';
  } else {
    displayOrderLink.innerHTML = 'To nie jest numer zamówienia';
  }
};

skuListButton.addEventListener('click', skuListToCopyInput);
skuLinkButton.addEventListener('click', skuListLinkInput);
skuListFromMessageButton.addEventListener('click', skuListToCopyMessage);
skuLinkFromMessageButton.addEventListener('click', skuListLinkMessage);
orderLinkButton.addEventListener('click', orderLink);
