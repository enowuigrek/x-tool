...
const delateDuplicateAndUndefined = (list) => {
  list = [...new Set(list)];
...
const listToCopy = (list) => {
  clearlistSkuArr();
  delateDuplicateAndUndefined(list);
...
const linkToList = (list) => {
  clearlistSkuArr();
  generatelistSkuArrArray();
  delateDuplicateAndUndefined(list);
...
generateSkuButton.addEventListener('click', displaySkuListToCopy(generatelistSkuArrArray));
