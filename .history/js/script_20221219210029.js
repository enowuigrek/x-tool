...
const generateSkuFromLinks = () => {
  ...
  for (let pieceOfInputText of splitInputTextArray) {
    ...
    if (pieceOfInputText.includes('https://www.x-kom.pl/p/')) {
      ...
      listCorrectSku.push(skuFromInputLink);

    } else {
      const foundXKomIndex = splitInputTextArray.indexOf('x-kom:')
      console.log(foundXKomIndex);
      console.log(splitInputTextArray[foundXKomIndex+1])
      const  = splitInputTextArray[foundXKomIndex+1]
      listCorrectSku.push(skuFromInputLink);
      
      // const skuFromInputText = 
      // pieceOfInputText = pieceOfInputText.replace('kod x-kom: ','-----')
      // const splitKod = pieceOfInputText.split('-----');
      // listCorrectSku.push(splitKod[1])
    }
  }
