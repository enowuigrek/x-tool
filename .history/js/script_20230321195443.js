...
const copyskuListButton = document.getElementById('.copy_sku_list');
const copyListFromMessageButton = document.getElementById('.copy');
...
const displaySkuListToCopyInput = () => {
  ...
  copyButton.classList.remove('no_active');
  ...
  if (!resultSkuFromInput.innerHTML) {
    ...
    copyButton.classList.add('no_active');
  }
};
...
if (resultSkuFromInput.innerHTML == '') {
  console.log(resultSkuFromInput.innerHTML)
}
...
const copySku = (result) => {
  ...
  if (
      result.innerHTML != 'Wklej sku oddzielone spacją lub enterem' ||
      result.innerHTML != 'Nie znalazłem sku w tym tekście'
    ) {
