...
for (var i = 0; i < productListItems.length; i++) {
  ...
  if (quantityDropdown !== null) {
    productQuantity = quantityDropdown.textContent;} else {productQuantity = productListItems[i].querySelector('.disabledDropdownInputAmount').textContent;}finalList[i] = { sku: productSKU, quantity: productQuantity }
