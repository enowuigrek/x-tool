...
const generateSkuFromLinks = () => {
  ...
  for (let pieceOfInputText of splitInputTextArray) {
    ...
    if (pieceOfInputText == 'x-kom:') {
       ...
       console.log('tablica przed akcją: ' + splitInputTextArray);

        const foundXKomIndex = splitInputTextArray.indexOf(pieceOfInputText)
        console.log('index x-kom: '+ foundXKomIndex);
        splitInputTextArray[foundXKomIndex] = 'delated-x-kom';
        console.log(splitInputTextArray);
        console.log('to co zostanie wepchnięte do dablicy, po usunięciu x-kom:' + splitInputTextArray[foundXKomIndex+1]);
        skuFromTextArray.push(splitInputTextArray[foundXKomIndex+1]);
